...
async function saveAuthor() {
  if (!name.trim()) {
    setNameError("Please enter the author name");
    return;
  }
  ...
  try {
    const response = await axios.post(`${API_URL}/author`, formData, {
      headers: {
        Authorization: `Bearer ${token}`,
...
const handleStatusChange = async (id)=>{
  MySwal.fire({
   ...
   try{
      const response = await axios.get(`${API_URL}/author/status/${id}`, {
        headers:{
          Authorization : `Bearer ${token}`,
          "Content-Type" : 'application/json',
          Accept : 'application/json',
    ...
    {loading && (
      ...
      <div className="page-header">
        <div className="row align-items-center">
          <div className="col">
            <h3 className="page-title">Author</h3>
                        ...
                        <td className="text-end">
                            <div className="actions ">
                              {editId === v.id ? ( // If editing, show update button
                                ...
                                <button
                                  onClick={() => handleEdit(v.id, v.author_name)}
